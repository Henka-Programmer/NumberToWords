...
$solution_file = Join-Path $path "number-to-words.sln"
...
target deploy {
  ...
  # Copy to output dir
  Copy-Item "$path\src\NumberToWords\bin\$configuration" -Destination "$output_dir\NumberToWords" -Recurse
 }
...
$result = 1
  ...
  elseif ($result -eq 1) {
    $packages | foreach {
      ...
      Invoke-Dotnet nuget push $package --api-key $key --source "https://api.nuget.org/v3/index.json"
      write-host
    }
  }
}
...
target decrypt-private-key {
  if ((Test-Path ENV:kek) -and ($ENV:kek.Length -gt 0) -and ($env:SYSTEM_PULLREQUEST.ISFORK -eq $false)) {
    ...
    dotnet "appveyor-tools/secure-file.dll" -decrypt src/NumberToWords-Release.snk.enc -secret $ENV:kek
    if (($LASTEXITCODE -eq 0) -and (Test-Path "$path/src/NumberToWords-Release.snk")) {
      $script:keyfile = "$path/src/NumberToWords-Release.snk";
...
function verify_assembly($path) {
  ...
  return $token -eq "fa850e4d7c78239a";
}
